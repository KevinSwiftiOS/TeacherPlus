...
import { OverViewPage } from "../../MyCourses/OverViewPage/OverViewPage";
import { HttpService } from "../../../services/Commons/HttpService";
import { JPush } from '@jiguang-ionic/jpush';
import { Device } from '@ionic-native/device';
declare var swal;
@Component({
  selector: 'page-MyCourseTabPage',
  templateUrl: 'MyCourseTabPage.html',
  providers: [HttpService]
})
export class MyCourseTabPage {
  items = new Array();
  isDemo = false;
  ...
  //   alert('Error!' + '\nSequence: ' + sequence + '\nCode: ' + code);
  // };
  constructor(public navCtrl: NavController, private httpService: HttpService, public jpush: JPush, device: Device) {
    ...
    this.isDemo = false;
    this.httpService.request("api/projects", "", true).then(
      res => {
  ...
  //打印证书
  PrintCer(item) {
    this.httpService.openBrowser(item.printCerUrl);
  }
  //上拉刷新
  doRefresh(refresher) {
    this.httpService.request("api/projects", "", false).then(
      res => {
        refresher.complete();
        if (res.code == 0) {
          this.items = res.data;
